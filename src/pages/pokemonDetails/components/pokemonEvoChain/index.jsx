...
export const EvolutionChain = ({ evolutionChain, name }) => {
  ...
  const handleItemClick = (itemName) => {
    navigate({
      to: "/itemDetails/$name",
      params: { name: itemName }, // Passa o nome do item como parâmetro para a rota de detalhes
    });
  };
                    ...
                    {evolution.evolve
                      .filter(
                        (value, index, self) =>
                          ...
                          self.findIndex(
                            (v) =>
                              v.trigger === value.trigger &&
                              v.time_of_day === value.time_of_day,
                          ),
                      )
                      .map((evo, evoIndex) => {
                        const conditions = [
                          evo.trigger
                            ? `Trigger: ${formatName(evo.trigger)} `
                            : null,
                          evo.min_level ? `Min Level: ${evo.min_level}` : null,
                          evo.time_of_day
                            ? `Time of Day: ${formatName(evo.time_of_day)}`
                            : null,
                          evo.turn_upside_down ? "Turn Upside Down" : null,
                          evo.party_type_id
                            ? `With a ${formatName(TYPE_MAP[evo.party_type_id])} type in party.`
                            : null,
                          evo.party_species_id === 223
                            ? `With Remoraid in party.`
                            : null,
                          evo.trade_species_id === 588
                            ? `Trade for Karrablast`
                            : evo.trade_species_id === 616
                              ? `Trade for Shelmet`
                              : null,
                          evo.relative_physical_stats === 1
                            ? "Level up with Attack > Defense"
                            : evo.relative_physical_stats === -1
                              ? "Level up with Attack < Defense"
                              : evo.relative_physical_stats === 0
                                ? "Level up with Attack = Defense"
                                : null,
                          evo.gender_id === 1
                            ? "Female"
                          ...
                          evo.heldItem ? (
                            <span>
                              ...
                              <a
                                ...
                                onClick={(e) => {
                                  e.preventDefault();
                                  handleItemClick(evo.heldItem);
                                }}
                              >
                                {formatName(evo.heldItem)}
                              </a>
                            </span>
                          ) : null,
                          evo.location
