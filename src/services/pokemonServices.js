...
export const findPokemonFullData = async (name) => {
  ...
  const evolutionTree =
    raw.specy?.chain?.species.map((item) => {
      ...
      return {
        ...
        evolve: item.evolutions_details.map((evolution) => ({
          ...
          heldItemSprite:
            evolution.heldItem?.sprites[0]?.sprites?.default || null,
  ...
  return {
    ...
    description:
      species?.flavorTexts[0]?.flavor_text || "No description available",
